...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (setq org-agenda-sorting-strategyq
...
** 根据事项耗时在agenda中显示不同的高度
#+begin_src emacs-lisp
  ;; 摘自https://emacs-china.org/t/org-agenda/8679
  ;; 30分钟以下都用正常的一行显示，30分钟以上按比例扩展行高，
  ;; (defun my/org-agenda-time-grid-spacing ()
  ;;   "Set different line spacing w.r.t. time duration."
  ;;   (save-excursion
  ;;     (let* ((background-color (alist-get 'background-color (frame-parameters)))
  ;;            (alphas '(100 99))
  ;;            pos
  ;;            duration)
  ;;       (nconc alphas alphas)
  ...
  ;;             ;; (overlay-put ov 'alpha alpha)
  ;;             (overlay-put ov 'face `(:background ,(color-lighten-name background-color alpha)))
  ;;             (setq alphas (cdr alphas))
  ...
  (defun my/org-agenda-time-grid-spacing ()
    ...
    (save-excursion
      (let* ((background (alist-get 'background-mode (frame-parameters)))
             (background-color (alist-get 'background-color (frame-parameters)))
             (background-dark-p (string= background "dark"))
             (colors (if background-dark-p
                         (list background-color "DarkSlateGray")
                       (list background-color "#F6B1C3" "#FFFF9D" "#BEEB9F" "#ADD5F7")))
             pos
             duration)
        (nconc colors colors)
        (goto-char (point-min))
   ...
   * A HOLD task
   ...
   #+BEGIN_SRC emacs-lisp
     (setq org-todo-keywords
           (quote ((type "TODO(t)" "NEXT(n)" "PROG(p)" "TODAY(T)" "WAITING(w@/!)" "|" "DONE(d)" "CANCELLED(c@/!)" "SUSPEND(s@/!)"))))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; * Moving a task to PROG removes WAITING and add HOLD tags
...
*** 当 task 开始 clock in 时，自动将状态变为 PROG
#+BEGIN_SRC emacs-lisp
   (defun bh/mark-tasks-prog ()
    "change states to PROG"
    (org-todo "PROG"))

  (add-hook 'org-clock-in-hook #'bh/mark-tasks-prog 'append)
#+END_SRC
...
若安装了 anki-editor 则添加“卡片”的抓取快捷键
#+begin_src emacs-lisp
  (when (package-installed-p 'anki-editor)
    (require 'anki-editor)
    (defun get-anki-address ()
      "Get anki listening address using DISPLAY environment variable"
      (let* ((DISPLAY (getenv "DISPLAY"))
             (host (nth 0 (split-string DISPLAY ":"))))
        (if (string-empty-p host)
            "127.0.0.1"
          host)))
    (setq anki-editor-anki-connect-listening-address (get-anki-address))
    ...
    (add-to-list 'org-capture-templates
                 `("cp" "Person"
                   entry (file+headline ,(expand-file-name "cards.org" MY-NOTE-PATH) "Person")
...
#+RESULTS:
(:export org-video-link-export :follow org-video-link-follow)
